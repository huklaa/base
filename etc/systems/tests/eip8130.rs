...
const PROOF_PROGRESS_INTERVAL: Duration = Duration::from_secs(30);
...
/// `RUST_LOG=info cargo nextest run --release -p base-system-tests --run-ignored all -E 'test(eip8130_block_dry_run)' --no-capture`
...
async fn eip8130_block_dry_run_proves() -> Result<()> {
    let _ = tracing_subscriber::fmt()
        ...
        "SP1 dry-run execute does not finish in the unoptimized test profile. Re-run with --release:\n\
         RUST_LOG=info cargo nextest run --release -p base-system-tests --run-ignored all \\\n\
         -E 'test(eip8130_block_dry_run)' --no-capture"
    );

    info!("starting Cobalt stack with forced batch submission");
    let (system, provider) =
        common::start_cobalt_stack(SystemTestStackBuilder::new().with_force_batch_submission())
            .await?;
    info!("stack is up; sending EIP-8130 transaction");
    let (tx_hash, receipt) = send_minimal_eip8130(&provider).await?;
    let block_number =
        receipt.block_number().expect("mined EIP-8130 transaction must have a block");
    info!(tx_hash = %tx_hash, block_number, "EIP-8130 transaction mined");

    let rollup_provider =
        RootProvider::<Base>::new_http(system.l2_stack().builder_consensus_rpc_url());
    let l1_head = wait_for_safe_l2(&rollup_provider, block_number).await?;
    info!(block_number, l1_head = %l1_head, "EIP-8130 block is safe");

    info!("starting in-process prover-service");
    let service = InProcessProverService::start().await?;
    info!(url = %service.url(), "starting in-process ZK host");
...
async fn send_minimal_eip8130(
    ...
    common::wait_for_balance(provider, signer.address()).await?;
    ...
    let receipt = provider
        .wait_for_receipt(tx_hash, common::TX_RECEIPT_TIMEOUT)
        .await
        .wrap_err("EIP-8130 receipt timed out")?;
    Ok((tx_hash, receipt))
}

async fn wait_for_safe_l2(provider: &RootProvider<Base>, block_number: u64) -> Result<B256> {
    info!(block_number, "waiting for EIP-8130 block to become safe");
    match timeout(SAFE_L2_TIMEOUT, async {
        ...
        loop {
            ...
            if last_progress.elapsed() >= PROOF_PROGRESS_INTERVAL {
                info!(
                    block_number,
                    safe_l2 = status.safe_l2.number,
                    unsafe_l2 = status.unsafe_l2.number,
                    elapsed_s = started.elapsed().as_secs(),
                    "waiting for EIP-8130 block to become safe"
                );
...
async fn poll_dry_run_stats(
    ...
    match timeout(PROOF_TIMEOUT, async {
        ...
        loop {
            ...
            let status_changed = last_status != Some(response.status);
            if status_changed || last_progress.elapsed() >= PROOF_PROGRESS_INTERVAL {
                info!(
                    session_id = %session_id,
                    status = ?response.status,
                    elapsed_s = started.elapsed().as_secs(),
                    "waiting for dry-run proof"
                );
