...
impl BootInfo {
        ...
        O: PreimageOracleClient + Send,
    {
        ...
        let l2_schedule_timestamp = rollup_config
            .checked_l2_block_timestamp(schedule_l2_block_number)
            .ok_or(OracleProviderError::L2ScheduleTimestampOverflow {
                schedule_block: schedule_l2_block_number,
            })?;
        let l2_claim_timestamp = rollup_config
            .checked_l2_block_timestamp(l2_claim_block)
...
mod tests {
    ...
    async fn rejects_l2_schedule_timestamp_overflow() {
        ...
        rollup_config.genesis.l2_time = u64::MAX - 1;
        ...
        oracle.insert(L2_OUTPUT_ROOT_KEY, B256::repeat_byte(0x22).to_vec());
        oracle.insert(L2_CLAIM_KEY, B256::repeat_byte(0x33).to_vec());
        oracle.insert(L2_CLAIM_BLOCK_NUMBER_KEY, 0u64.to_be_bytes().to_vec());
        oracle.insert(L2_SCHEDULE_BLOCK_NUMBER_KEY, 1u64.to_be_bytes().to_vec());
        oracle.insert_rollup_config(ORACLE_CHAIN_ID, &rollup_config);

        let err = BootInfo::load(&oracle).await.expect_err("overflowing timestamp should fail");
        assert!(matches!(
            err,
            OracleProviderError::L2ScheduleTimestampOverflow { schedule_block: 1 }
        ));
    }
