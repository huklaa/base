...
impl RollupNode {
    ...
    ///
    /// The data directory is the one holding the checkpoint database, which is the disk whose
    /// filling up would stop this node.
    fn telemetry_facts(&self, mode: NodeMode) -> TelemetryNodeFacts {
        TelemetryNodeFacts {
            ...
            data_dir: self.checkpoint_path.parent().map(Path::to_path_buf),
