    ...
    pub async fn has_unread_expired(&self, cutoff: DateTime<Utc>) -> Result<bool> {
        query_scalar(
            "SELECT EXISTS ( \
               ...
               WHERE blocks.reorged_out \
                 AND blocks.updated_at < $1 \
                 AND (blocks.updated_at, blocks.number, blocks.hash) \
                     > (cursor_row.last_updated_at, \
                        cursor_row.last_number, \
                        cursor_row.last_hash) \
             )",
        )
        .bind(cutoff)
        .fetch_one(&self.pool)
    ...
    async fn delete_batch(conn: &mut PgConnection, cutoff: DateTime<Utc>) -> Result<u64> {
        ...
        let deleted = query(
            "WITH expired AS MATERIALIZED ( \
               SELECT number, hash FROM shadow_blocks \
               WHERE updated_at < $1 \
             ...
             USING expired \
             WHERE blocks.number = expired.number AND blocks.hash = expired.hash",
        )
        .bind(cutoff)
