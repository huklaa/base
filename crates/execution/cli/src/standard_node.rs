...
const DEFAULT_SHADOW_INDEXER_RETENTION: &str = "7d";
...
mod tests {
    ...
    #[test]
    fn test_shadow_indexer_retention_defaults_to_seven_days() {
        let args = CommandParser::<StandardNodeArgs>::parse_from([
            "reth",
            "--enable-shadow-indexer",
            "--shadow-indexer.database-url",
        ...
        assert_eq!(config.retention.period, Duration::from_secs(7 * 24 * 60 * 60));
    ...
    fn test_shadow_indexer_rejects_zero_retention() {
        let args = ShadowIndexerArgs {
            ...
            shadow_indexer_database_url: Some("postgres://localhost/shadow".to_string()),
